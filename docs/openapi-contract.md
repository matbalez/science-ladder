...
- `GET /v1/me` → `{user:null|{id,githubId,login,avatarUrl,role,invited},quotas:{remaining:null,lifetimeLimit:null,activeLimit},capabilities:{creation,submission,review},configuration:{githubAuth,scientificReview,platformRunner,independentRunner,officialRunner}}`.
...
- `GET /v1/dashboard` → `{challenges:Challenge[],candidates:Candidate[],submissions:Submission[],intents:Intent[],participation:Participation[],submissionCount:number,submissionContexts:SubmissionContext[]}` (invited owner only). Participation aggregates the owner’s submission attempts by challenge across versions, with `id`, `slug`, `title`, latest visible `status`, `open`, all-time `submissionCount`, unaccepted/nonfailed `pendingCount`, and `lastActivity`. `open` requires published status, open intake, and a future deadline; this does not track local solver activity. `submissionCount` is the all-time total, even though the submission list is limited to the latest 100. `submissionContexts` supplies the exact version’s title, slug, quantum and units for those recent rows.
...
- `POST /v1/invites` with `{githubUsername:"octocat",role:"member"|"editor"}` → `{githubId,githubUsername,role}` (operator). Usernames are resolved through GitHub and invitations remain bound to the returned stable account ID. Whitespace, a leading @ and case differences are accepted. Unknown usernames and organization accounts are rejected without granting access. The numeric `githubId` input remains accepted for legacy API clients; do not send both identity fields. The retired `validationQuota` input is accepted but ignored for old clients. There is no lifetime allowance; account concurrency, daily preparation/admission budgets and infrastructure capacity still apply.
