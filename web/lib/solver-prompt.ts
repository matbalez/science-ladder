...
export const CLI_SOURCE = "cd73a393ff12a3ec22f3b9c5e7c5c0ea0a40cfcc";
  ...
  asText(asRecord(m.evaluation).mode) === "performance"
    ...
    : nativeQuietEchoes
      ...
      : nativeMultiply || nativeTriangle || nativeLoadPaths
        ? `sl claim --api ${quote(api)} --version ${quote(c.versionId)} --artifact ../candidate-artifact --out "$SL_FINAL_RUN/claim.json" --report "$SL_FINAL_RUN/result.json" --checked-gates ${nativeMultiply ? "exact-tensor-identity,ordered-bilinear-products,bounded-certificate" : nativeTriangle ? "unit-square,all-triples,exact-arithmetic" : "physical-design,fixed-material,equilibrium"} -- python3 local.py --solver ../candidate-artifact/solver.py --output "$SL_FINAL_RUN/result.json"`
        : nativeProgram
