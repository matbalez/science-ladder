...
- `POST /v1/invites` with `{githubId,role:"member"|"editor"}` → `{githubId,role}` (operator). The retired `validationQuota` input is accepted but ignored for old clients. There is no lifetime allowance; account concurrency, daily preparation/admission budgets and infrastructure capacity still apply.
