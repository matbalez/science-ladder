# Science Ladder Challenge Scout · 1.9.0
...
## Local solver workflow

Include `science-ladder-local.json` in the source before publication. Follow https://scienceladder.org/docs/cli and the repository docs/cli.md recipe specification: version 1, setup argument arrays, baseline and check argument arrays writing a fresh `{report}`, and check consuming `{artifact}`. Commands execute from the frozen challenge source with no implicit shell. Supply a repeatable setup script, documented runtime versions, exact expected baseline and public hard-gate checks. Prefer native local validation without Docker when feasible. The recipe is convenience metadata pinned by the source commit; it cannot redefine scientific scoring or attest hidden-test/hardware results. Test fresh-clone `sl setup`, `sl run --baseline`, valid and invalid candidates, and the non-improvement submission rejection. Do not require agents to build the CLI from source: direct them to prebuilt macOS/Linux binaries.
